  ...
  return {
    ...
    samples: {
      bad: Array.from({ length: 5 }, (_, index) => ({
        verdict: "fail",
        logs: [long ? `ReproAssertionError: sample ${index}` : "ReproAssertionError: regression"],
        durationMs: 12 + index,
    ...
    timeline: [
      { at: signedAt, event: "run", detail: long ? suffix : "STABLE_LOCAL_DIFFERENTIAL" },
...
async function testPersistenceFailure(page, baseUrl, errorName) {
  ...
  assert.equal(result.run.reason, "STABLE_LOCAL_DIFFERENTIAL");
...
async function encodeOnPage(page, artifact) {
  return page.evaluate(async value => {
    const { encodeReceipt, prepareReceiptShare } = await import("/src/inbox/receipt.js");
    const review = await prepareReceiptShare(value);
...
async function testReceiptHashChanges(page, baseUrl) {
  ...
  const validArtifact = structuredClone(fixture);
  validArtifact.reproSha256 = sha256(validArtifact.repro);
  const valid = await encodeOnPage(page, validArtifact);
  ...
  assert.doesNotMatch(synchronous.text, /Repro hash self-consistent ✓/);
  await page.getByText("Repro hash not self-consistent ✗", { exact: true }).waitFor();

  synchronous = await replaceHashAndDispatch(page, validHash);
  assert.equal(synchronous.status, "Verifying receipt…");
  assert.doesNotMatch(synchronous.text, /Repro hash not self-consistent ✗/);
  await page.getByText("Repro hash self-consistent ✓", { exact: true }).waitFor();
  ...
  await page.getByText("Repro hash self-consistent ✓", { exact: true }).waitFor();
  assert.doesNotMatch(await page.locator("#receipt-root").textContent(), /Receipt could not be opened|Repro hash not self-consistent ✗/);
}
